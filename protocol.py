import asyncio  # ✅ FIX: كان مفقوداً — بسببه كان ĐukaCore يرفع NameError عند التهيئة
...
class ĐukaMessage:
    ...
    def from_bytes(cls, data: bytes) -> 'ĐukaMessage':
        ...
        # ✅ FIX: تحويل component من string إلى ComponentType عند الاستيراد
        if isinstance(d.get('component'), str):
            d['component'] = ComponentType[d['component'].split('.')[-1]]
        return cls(**d)
...
class ĐukaCore:
    ...
    def __init__(self, node_id: str):
        ...
        self.message_queue: asyncio.Queue = asyncio.Queue()  # ✅ يعمل الآن
        self.running = False
